...
object SunSTCommand : Initable {
    ...
    override fun init() {
        SCommand("SunST", "sun")
            .addCommand("machine", "多方块机器") { wrapper ->
                wrapper.apply {
                    "book" {
                        empty {
            ...
            .addCommand("give", "获得一个SunST物品", isOp = true) { wrapper ->
                wrapper.apply {
                    SItem.items.keys {
                        empty {
                            if(sender !is Player){
            ...
            .addCommand("reload", "重载配置文件", isOp = true) { wrapper ->
                wrapper.apply {
                    empty {
            ...
            .addCommand("item", "手持物品编辑", isOp = true) { wrapper ->
                wrapper.apply {
                    empty {
                        val player = getPlayer() ?: return@empty
